...
bool toogle_serial_in_photo_hand = 0;
bool toogle_serial_in_photo_bottle = 0;
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(IN_PHOTO_HAND), change_photo_hand_sensor_state, CHANGE);
  // ----
  // CZUJNIK NA KONCU TASMY KONFIGURACJA
  attachInterrupt(digitalPinToInterrupt(IN_PHOTO_BOTTLE), change_photo_bottle_sensor_state, CHANGE);  
  // ----
...
void loop() {
  ...
  if (toogle_serial_in_photo_hand == 1){
    serial_in_photo_hand();
  }

  if (toogle_serial_in_photo_bottle == 1){
    serial_in_photo_bottle();
  }



  if (licznik_led == 20000)
  {
    licznik_led = 0;
  ...
  {
    int suma1 = BT1.get_units()+BT2.get_units();
    int suma2 = BT1.get_units()+BT2.get_units();
    int suma3 = BT1.get_units()+BT2.get_units();
    int suma4 = BT1.get_units()+BT2.get_units();
    int suma5 = BT1.get_units()+BT2.get_units();

    int suma = (suma1+suma2+suma3+suma4+suma5)/5;
    String value = "";
    if (suma > 999999) 
    {
...
void serial_in_photo_hand(){
  toggle_led = !toggle_led;
  digitalWrite(LED_BUILTIN, toggle_led ? HIGH : LOW);
  if (digitalRead(IN_PHOTO_HAND))
    {
      Serial.print("f0f******");
      toogle_serial_in_photo_hand = 0;     
    }
  else  
    {
      Serial.print("f1f******");
...
void serial_in_photo_bottle(){

  if (digitalRead(IN_PHOTO_BOTTLE))
    {
      Serial.print("g0g******");
      toogle_serial_in_photo_bottle = 0;    
    }
  else
    {
      Serial.print("g1g******");
...
{  
  toogle_serial_in_photo_bottle = 1;
}
