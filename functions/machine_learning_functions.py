...
def load_cams():
    try:
        CAMERA_UP = cv2.VideoCapture(config.CAM_UP_INDEX, cv2.CAP_DSHOW)
        CAMERA_LID = cv2.VideoCapture(config.CAM_LID_INDEX, cv2.CAP_DSHOW)
        print(c.OK_GREEN+"Machine learning: Cameras loaded!"+c.ENDC)
        return CAMERA_UP, CAMERA_LID
    except:
        print(c.ERROR+ "Machine learning: Failed to load cameras!" + c.ENDC)
        return False
def load_models():
    print("Machine learning: Loading model files...")
    try:
        MODEL_UP = load_model('ml_files/model_up.h5')
        MODEL_LID = load_model('ml_files/model_lid.h5')
        print(c.OK_GREEN + "Machine learning: Model files loaded!" + c.ENDC)
        return MODEL_UP,MODEL_LID
    except:
        print(c.ERROR + "Machine learning: Failed to load model files'! Aborting camera_analysis_worker()" + c.ENDC)
        return False
...
def camera_analysis_worker(machine_class):
    print("Machine learning: Starting camera analysis worker...")
 
    try:
        camera_up = cv2.VideoCapture(config.CAM_UP_INDEX)
        camera_lid = cv2.VideoCapture(config.CAM_LID_INDEX)
    except:
        print(c.ERROR + "Machine learning: Failed to capture cams! Aborting camera_analysis_worker()" + c.ENDC)
        return False
 

    print(c.OK_GREEN+"Machine learning: Camera analysis worker started!"+c.ENDC)
    while True:
        ...
        if not check_probabilities_idle(probabilities_up,probabilities_lid):
            print(c.WARNING+"Machine learning: Activity on conveyor belt detected!"+c.ENDC)
